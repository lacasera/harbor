...
- **New tunnel provider** → add `src/main/tunnels/<id>.ts` implementing `TunnelDriver`, register it in `tunnels/index.ts`. Its `plan()` MUST preserve the project domain to the origin (Host header, and TLS SNI when secured) or nginx serves the catch-all vhost, not the project — see `src/main/tunnels/types.ts`. Tunnels are never persisted or auto-started, and are torn down in `HarborApp.shutdown` before `processes.stopAll`.

## Verifying without harming the live environment

Harbor serves the captain's own machine; `~/.harbor` is his working state, not a fixture. `smoke` and `verify:tunnel` are self-contained (no HarborApp, no `~/.harbor`); the other `verify:*` scripts construct a real `HarborApp` and act on the live state (download binaries, start services), so treat their failures as possibly environmental and avoid running them casually.
