...
export class NginxManager {
  ...
  /**
   * The catch-all vhost, named to sort first so nginx treats it as the default
   * server for Harbor's ports.
   *
   * Without it, a request for a host with no matching server block is served
   * by whichever vhost nginx loaded first — so a site whose vhost is missing,
   * or on the wrong port, silently returns a *different project's* content.
   * That is far worse than an error: nothing looks broken, the wrong thing is
   * just quietly true.
   */
  defaultVhostPath(): string {
  ...
  writeDefaultVhost(options: {
    ...
    const body = [
      ...
      // 404 rather than a redirect or a stub site: the honest answer is that
      // Harbor has nothing for this host.
      `return 404 "Harbor: no site is configured for this host.\n\nThe request reached Harbor's nginx but matched no .${options.tld} site.\nCheck the project is parked and its vhost was written.\n";`
    ]

    const blocks = [
      '# Generated by Harbor — catch-all for hosts with no site. Do not edit.',
      'server {',
      ...indent([`listen ${options.httpPort};`, `listen [::]:${options.httpPort};`, ...body]),
      '}',
      ''
    ]
