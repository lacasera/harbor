...
- [x] **6.1 — Icon and build assets.** *(done)*
  The icon is generated by `npm run icon` rather than committed as an opaque
  binary, so it is reviewable in a diff and regenerable at any size. Packaging
  is proven: `npm run package` produces a 269 MB arm64 `.app` that launches.

- [ ] **6.2 — Signing + notarization.** **Blocked on a certificate.**
  This machine has an *Apple Development* certificate, which signs for local
  use but is refused by Gatekeeper elsewhere. Distribution needs a *Developer ID
  Application* certificate plus notarization credentials. `npm run
  doctor:release` reports exactly what is present and what is missing rather
  than failing deep inside a packaging run. Notarization is commented out in
  `electron-builder.yml` on purpose — enabling it without credentials fails the
  build instead of silently shipping something unnotarized.

- [ ] **6.3 — `SMJobBless` helper.** Deliberately deferred, and cheaper than it
  was: the only privileged operations left in the entire app are writing
  `/etc/resolver/<tld>` and the nginx.conf fallback. Connecting nginx and all of
  TLS turned out to need no root at all. It is also blocked on 6.2, since the
  helper and app must be signed with matching requirements. `sudo-prompt` is
  what `CLAUDE.md` specifies for v1 and is adequate for one setup-time prompt.

- [x] **6.4 — Update checking.** *(wired, unverifiable here)*
  Checked on demand from Settings, never in the background. Every outcome is
  reported rather than thrown, and it lives outside `HarborApp` so the backend
  stays importable without `electron` for the verification scripts.
  The download/install path is **untested**: it needs a signed public release to
  check against, and the repo is private. Treat it as unproven until then.
...
| Command | Covers |
|---|---|
| `npm run smoke` | 23 pure-logic checks — vhost rendering, nginx config surgery, detection order, version matching, schema validation, both ERD analyzers |
| `npm run verify:slice` | park → dev server → nginx → DNS → TLS, end to end (16 steps) |
